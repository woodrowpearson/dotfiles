...
# Build the dotfiles
mkdir $HOME/.config
git clone https://github.com/woodrowpearson/dotfiles $HOME/.config/nix
nix build "$HOME/.config/nix#darwinConfigurations.WoodyMBP.system" --extra-experimental-features "nix-command flakes"

# Remove default nix config to mitigate https://github.com/LnL7/nix-darwin/issues/149
sudo rm -f /etc/nix/nix.conf
./result/sw/bin/darwin-rebuild switch --flake "$HOME/.config/nix#WoodyMBP"
```

After first successful deployment, use the following command to switch:

```console
darwin-rebuild switch --flake "$HOME/.config/nix#WoodyMBP"
```
